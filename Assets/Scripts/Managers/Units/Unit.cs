...
public class Unit : MonoBehaviour, IDamagable
{
    ...
    public int GetMaxMovement => _maxMovement;
    ...
    public event Action<Unit> OnMovementChanged;
    private void Awake()
    {
    ...
    public bool CanSpend(int cost, bool isAP = true) => (isAP ? _ap : _movementPoints) >= cost;
    ...
    public bool SpendMovement(int cost, bool spendNow = true)
    {
        if (!CanSpend(cost, false))
            return false;
        if (spendNow)
        {
            _movementPoints -= cost;
            OnMovementChanged?.Invoke(this);
        }
        return true;
    }
